...
function showText(cameraPosition) {
  ...
  if(cameraPosition.x <= mercury.planet.position.x) {
    // console.log("sun");
    textDiv.innerHTML = "S&nbsp;&nbsp;U&nbsp;&nbsp;N";
  } else if( cameraPosition.x <= venus.planet.position.x) {
    // console.log("mercury");
    textDiv.innerHTML = "M&nbsp;&nbsp;E&nbsp;&nbsp;R&nbsp;&nbsp;C&nbsp;&nbsp;U&nbsp;&nbsp;R&nbsp;&nbsp;Y";
  } else if( cameraPosition.x <= earth.planet.position.x) {
    // console.log("venus");
    textDiv.innerHTML = "V&nbsp;&nbsp;E&nbsp;&nbsp;N&nbsp;&nbsp;U&nbsp;&nbsp;S";
  } else if( cameraPosition.x <= mars.planet.position.x) {
    // console.log("earth");
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key === "ArrowRight") {
    ...
    if (camPosIndex < unknown) {
      ...
      // helmet.position.x = camPos.x;
      // helmet.position.y = camPos.y-10;
      // helmet.position.z = camPos.z;
      // // if(camera.position.x >= mars.mesh.position.x +100) {
        camera.rotation.x = camRot.x;
        camera.rotation.y = camRot.y;
        camera.rotation.z = camRot.z;
        // helmet.rotation.x = camRot.x;
        // helmet.rotation.y = camRot.y;
        // helmet.rotation.z = camRot.z;
        if(spline.getPoint((camPosIndex+smoothness) / unknown).z > spline.getPoint((camPosIndex) / unknown).z) {
          var lookAlong = spline.getTangent((camPosIndex-smoothness) / unknown);
          camera.lookAt(lookAlong);
          // helmet.lookAt(lookAlong);
          // helmet.position.x += -3;
          // helmet.rotation.x = camera.rotation.x;
          // helmet.rotation.z = camera.rotation.z;
        }
        else {
          var lookAlong = spline.getTangent((camPosIndex+smoothness) / unknown);
          camera.lookAt(lookAlong);
    ...
    if (camPosIndex < unknown && camPosIndex > 0) {
      ...
      if(spline.getPoint((camPosIndex+smoothness) / unknown).z > spline.getPoint((camPosIndex) / unknown).z){
        ...
        // helmet.lookAt(spline.getTangent((camPosIndex-smoothness) / unknown));
        // helmet.rotation.y = camera.rotation.y;
      } else {
        camera.lookAt(spline.getTangent((camPosIndex+smoothness) / unknown));
        // helmet.lookAt(spline.getTangent((camPosIndex-smoothness) / unknown));
        // helmet.rotation.y = camera.rotation.y;
